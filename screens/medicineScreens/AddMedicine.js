  ...
  const handleSave = () => {
    ...
    if(timestamp.includes("AM")){
      if(timestamp.startsWith("12")){
        ...
        time.setHours(Number.parseInt(timestamp.substring(0,2)), Number.parseInt(timestamp.substring(3,5)))
      }      
    }
    else{
      if(timestamp.startsWith("12")){
        ...
        time.setHours(Number.parseInt(timestamp.substring(0,2))+12, Number.parseInt(timestamp.substring(3,5)))
      }
    }
    
    MedicationService.addMedication(
      new Medication(0,nameMedicine, description, time.toISOString(), Number.parseInt(frequency))
    ).then((result) => {
      if (result === "") {
